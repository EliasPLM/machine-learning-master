...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=True, epsilon=1.0, alpha=0.02):
    ...
    def get_maxQ(self, state):
        ...
        # Find highest Q value in state
        for keys, value in self.Q[state].items():
            scores.append(value)
        max_score = max(scores)

        # compare highest Q to all actions scores in state and return all actions with highest Q
        for keys, value in self.Q[state].items():
            if value == max_score:
                maxQ.append(keys)
